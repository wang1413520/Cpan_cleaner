...
![扫描](docs/images/01-scan.png)

### 2. AI 标注（可选）—— 把看不懂的目录翻译成人话

![AI 标注](docs/images/02-ai-annotate.png)

### 3. 决策清单 —— 三色清单，每行都告诉你这是什么

![决策清单](docs/images/03-decision-list.png)

按判定筛选，只看「可迁」：

![可迁筛选](docs/images/04-migrate-filter.png)

### 4. 执行迁移 —— 先只读预检，确认无阻塞再动手

![预检](docs/images/05-preflight.png)

预检不通过的项**自动跳过**，其余照常迁移：

![迁移完成](docs/images/06-migrate-done.png)

### 5. 出问题随时回滚

![回滚](docs/images/07-rollback.png)
